...
function sameRequest(
  ...
  return (
    ...
    left.symbols.every(
      (symbol, index) => symbol === right.symbols[index],
    )
  );
}

function statusPresentation(status: MarketBarStatus): {
  label: string;
  icon: string;
  className: string;
} {
...
function validationMessages(error: ApiError): string[] {
  ...
  return Object.entries(errors).flatMap(([field, value]) => {
    ...
    return value
      ...
      .map((item) => `${humanizeCode(field) ?? field}: ${item}`);
  });
}
  ...
  const content = (
    <>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            {result.asset_id ?? "No resolved asset"}
          </p>
        </div>
      ...
      <p className="mt-3 text-xs text-slate-500">
        {result.bars.length} daily bar{result.bars.length === 1 ? "" : "s"}
      </p>
...
export function MarketDataExplorerPage() {
  ...
  const [submittedRequest, setSubmittedRequest] =
    useState<MarketBarQueryRequest | null>(null);
  const [selectedSymbol, setSelectedSymbol] = useState<string | null>(
    null,
  );
  const [formError, setFormError] = useState<string | null>(null);

  const normalizedSymbols = parseMarketSymbols(symbolInput);
  const marketQuery = useMarketBarQuery(submittedRequest);
  const result = marketQuery.data;

  const successfulResults =
    result?.results.filter(
      (symbolResult) => symbolResult.status === "SUCCEEDED",
    ) ?? [];
  ...
  const detailResult =
    successfulResults.find(
      (symbolResult) => symbolResult.symbol === selectedSymbol,
  ...
  function submitQuery(event: FormEvent<HTMLFormElement>) {
    ...
    if (symbols.length === 0) {
      setFormError("Enter at least one ticker symbol.");
      return;
    }
    ...
    if (sameRequest(submittedRequest, nextRequest)) {
  ...
  const apiError =
    marketQuery.error instanceof ApiError ? marketQuery.error : null;
  const apiValidationMessages =
    apiError?.status === 400 ? validationMessages(apiError) : [];
  ...
  return (
    <DashboardShell header={header}>
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="max-w-3xl">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600">
            Enter one or more canonical ticker symbols. Dates use an inclusive
            start and exclusive end. Results remain provider-neutral and no
            missing price is filled or interpolated in the browser.
          </p>
        </div>
        ...
        >
          <div className="lg:col-span-6">
            ...
            />
            <span id="ticker-help" className="mt-1 block text-xs text-slate-500">
              Commas, spaces, and new lines are accepted. Duplicates are removed
          ...
          <div className="lg:col-span-12">
            <p className="text-xs text-slate-500">
              Interval: <strong>Daily (1d)</strong> · Provider: server-configured default
            </p>
      ...
      ) : marketQuery.isPending && result === undefined ? (
        <div className="mt-5">
          <MarketDataSkeleton />
        </div>
      ) : marketQuery.error instanceof Error && result === undefined ? (
        <div className="mt-5">
          <StatePanel
            title={queryErrorTitle(marketQuery.error)}
          ...
          {apiValidationMessages.length > 0 ? (
            <ul className="mt-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-900">
              {apiValidationMessages.map((message) => (
        ...
        <div className="mt-5 space-y-5">
          {marketQuery.error instanceof Error ? (
            <div
              ...
              role="status"
            >
              Refresh failed. Showing the most recent successful market-data
              result.
            </div>
          ) : null}
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-1 text-lg font-semibold text-slate-950">
                  {result.meta.provider} · {result.meta.interval}
                </h2>
              </div>

              {successfulResults.length > 0 && unavailableResults.length > 0 ? (
            ...
            <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5">
              <div>
                ...
                <dd className="mt-1 font-medium text-slate-900">
                  {result.meta.start} to {result.meta.end} (end exclusive)
                </dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">Retrieved</dt>
                <dd className="mt-1 font-medium text-slate-900">
                  {formatDateTime(result.meta.retrieved_at)}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">Symbols</dt>
                <dd className="mt-1 font-medium tabular-nums text-slate-900">
                  {result.results.length}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">Canonical bars</dt>
                <dd className="mt-1 font-medium tabular-nums text-slate-900">
                  {result.row_count}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">Price view</dt>
                <dd className="mt-1 font-medium text-slate-900">
                  Raw close / OHLC · no normalization or fill
                </dd>
              </div>
            </dl>
          </section>
          ...
          {result.results.length === 0 ? (
            ...
            >
              ...
              <p className="mt-1 text-sm text-slate-500">
                Every requested symbol remains visible, including failures and
                no-data outcomes.
              </p>

              <ul className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {result.results.map((symbolResult) => (
                  <SymbolResultCard
                    key={symbolResult.symbol}
                    result={symbolResult}
                    selected={detailResult?.symbol === symbolResult.symbol}
                    onSelect={() => setSelectedSymbol(symbolResult.symbol)}
                  />
                ))}
              </ul>
            </section>
          ...
          {successfulResults.length > 0 ? (
            ...
            >
              ...
              <p className="mt-1 text-sm leading-6 text-slate-500">
                Provider-returned daily close price levels. This view is not
                normalized and does not fill or connect missing values.
              </p>

              <div className="mt-4">
                <MarketPriceChart results={successfulResults} />
              </div>
            </section>
          ) : null}
          ...
          {detailResult ? (
            ...
            >
              <h2
                ...
                className="text-lg font-semibold text-slate-950"
              >
                {detailResult.symbol} candlestick detail
              </h2>
              <p className="mt-1 text-sm leading-6 text-slate-500">
                Daily OHLC values are plotted in canonical ECharts order
                [open, close, low, high] with volume aligned to the same dates.
              </p>

              <div className="mt-4">
                <MarketCandlestickChart result={detailResult} />
              </div>
            </section>
          ) : null}
        </div>
