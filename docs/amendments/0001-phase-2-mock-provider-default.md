...
def test_unknown_default_provider_fails() -> None:
    ...
    assert (
        exc_info.value.code
        is ProviderConfigurationErrorCode.DEFAULT_PROVIDER_UNREGISTERED
    )
...
def test_unregistered_provider_cannot_be_enabled_by_configuration() -> None:
    ...
    assert (
        exc_info.value.code
        is ProviderConfigurationErrorCode.ALLOWED_PROVIDER_UNREGISTERED
    )
    assert exc_info.value.provider_name == "alpaca"
...
def test_default_provider_must_be_in_allowlist() -> None:
    ...
    assert (
        exc_info.value.code
        is ProviderConfigurationErrorCode.DEFAULT_PROVIDER_NOT_ALLOWED
    )
